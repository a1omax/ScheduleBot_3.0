...
def set_group(msg):
    ...
    gr_name = cur.execute("""SELECT group_name FROM schedule""").fetchall()
    ...
    for i in answer:
        for j in gr_name:
            if i == j[0]:
                ...
                bot.reply_to(msg, ("Группа успешно установлена на " + i))
                con.commit()
                return i

    else:
        bot.reply_to(msg, "Данной группы нет в базе или другая ошибка.")
        return 0
